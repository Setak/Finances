...
public class Excel {
    ...
    private HSSFWorkbook workbookWrite = new HSSFWorkbook();
    // Sets workbook from HSSF class, meaning it works with XLS files, use XSSF for XLSX files
    private HSSFSheet sheetWrite = workbookWrite.createSheet("FirstExcelSheet");
    // Sets first sheet in workbook and names it accordingly
    private HSSFRow rowWrite = sheetWrite.createRow(0);
    // Sets the row, 0 means it's the first one in the sheet
    private HSSFCell cellWrite = rowWrite.createCell(0);
